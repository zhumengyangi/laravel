...
class CreateJyGoodsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jy_goods', function (Blueprint $table) {
            ...
            $table->integer('brand_id')->default(0)->comment('商品id');
            ...
            $table->enum('is_shop',[1,2])->default('1')->comment('是否上架 1是 2否');
            $table->datetime('shop_time')->comment('上架时间');
            $table->enum('is_recommand',[1,2])->default('1')->comment('是否推荐 1是 2否');
            $table->enum('is_new',[1,2])->default('1')->comment('是否最新 1是 2否');
            $table->enum('is_hot',[1,2])->default('1')->comment('是否热销 1是 2否');
            $table->integer('sort')->default(100)->comment('商品排序');
            $table->integer('give_score')->default(0)->comment('商品送的积分');
            $table->enum('status',[1,2])->default('1')->comment('状态 1未审核 2已审核');
            $table->timestamps();
            $table->engine = 'InnoDB';
